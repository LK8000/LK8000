...
- **Map** — centred on the landable waypoint (airfield), with the same style as in Target view. Once **Direct** and a runway are selected, a **blue segment** is drawn on the map from the configured **direct leg** point on the extended centreline to the runway centre. **Default** length: **5 km** if distance units are not nautical miles, **3 NM** if units are **NM**. The bottom of the map may show distance and “PAN” (e.g. “DIRECT 34 1.5km PAN”).
...
- **Distance** (or short label in narrow layout) — length of the direct leg on the extended centreline. Adjust with **◀ ▶** (±1 km or ±0.5 NM); same style as numeric fields elsewhere (no slider drag).
- **Approve** — lower part of the panel (layout from the dialog). Opens the approval dialog to create the approach task (see below).

### 2.2 Map drawing

When **Direct** is selected and a runway is chosen, the map shows a **blue segment** from the outer end of the **direct leg** along the extended runway centreline (approach direction) to the **runway centre**. The leg length is the value in **Distance**. This is the same geometry used for the approach task.
...
4. Two buttons:
   - **Approve** — confirms. The current task is **replaced** by a two-point approach task:
     - **Point 1**: “DIRECT nn” (e.g. DIRECT 34) — the outer point of the direct leg on the extended centreline (same distance as **Distance** in the Approach dialog).
...
| **Approach** | Choose direct approach, runway, and direct leg length; view the segment on the map; optionally create the approach task with **Approve** and confirm in the warning popup. |
