...
public abstract class RK33xxEPDController {
	...
	public static boolean requestEpdMode(String epdMode) {
		try {
			Class.forName("android.view.View").getMethod("setByEinkUpdateMode", Integer.TYPE).invoke(null,
					getEpdMode(epdMode));
			return true;
		} catch (Exception e) {
	...
	private static int getEpdMode(String epdMode) {
		/*
		 * only full mode was tested, so return that. device owners will want to expand
		 * this to return other well known modes
		 */
		int mode = EPD_FULL;
