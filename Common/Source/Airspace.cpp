...
void ReadAirspace(ZZIP_FILE *fp)
{
  ...
  lastQNH = QNH;
	...
	// LKTOKEN  _@M399_ = "Loading Airspace File..." 
  ...
  if (NumberOfAirspacePointsPass[0] != NumberOfAirspacePointsPass[1]
    ...
    if (MessageBoxX(NULL, 
                    TEXT("Internal Airspace Parser Error!\r\nPlease send this Airspacefile to Support"), 
	// LKTOKEN  _@M68_ = "Airspace" 
                    gettext(TEXT("_@M68_")), MB_OKCANCEL) == IDCANCEL) {
    }
...
OnError:
  ...
  if (!bFillMode){
    ...
    wsprintf(sTmp, TEXT("%s: %d\r\n\"%s\"\r\n%s."),
             TEXT("Parse Error at Line"),
             LineCount, TempString,
             TEXT("Line skipped."));
	// LKTOKEN  _@M68_ = "Airspace" 
    if (MessageBoxX(hWndMainWindow, sTmp, gettext(TEXT("_@M68_")), 
      ...
      default:
        if (bFillMode){
          wsprintf(sTmp, TEXT("%s: %d\r\n\"%s\"\r\n%s."),
                   ...
                   TEXT("Line skipped."));
	// LKTOKEN  _@M68_ = "Airspace" 
          if (MessageBoxX(NULL, sTmp, gettext(TEXT("_@M68_")), 
                          MB_OKCANCEL) == IDCANCEL)
            return(-1);
        }
        continue;
