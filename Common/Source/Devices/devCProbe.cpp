...
BOOL CDevCProbe::ParseData(DeviceDescriptor_t* d, nmeastring& wiss, NMEA_INFO *pINFO ) {
	...
	double RelativeHumidity = int16toDouble(HexStrToInt(wiss.GetNextString())) * 0.1;
	pINFO->RelativeHumidity.update(*d, RelativeHumidity);
...
BOOL CDevCProbe::Config(DeviceDescriptor_t* d){
	...
	if(wf) {
    	...
    	wBt = wf->FindByName<WndButton>(TEXT("cmdSetCompassCal"));
    	if(wBt){
        	wBt->SetOnClickNotify(OnCompassCalClicked);
    	}
    	wBt = wf->FindByName<WndButton>(TEXT("cmdSetCalGyro"));
    	if(wBt){
        	wBt->SetOnClickNotify(OnCalGyroClicked);
    	}
    	wBt = wf->FindByName<WndButton>(TEXT("cmdZeroDeltaPress"));
    	if(wBt){
