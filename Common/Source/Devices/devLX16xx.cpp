...
TCHAR  szTmp[254];
  ...
  if (BaroAltitudeAvailable(GPS_INFO))
  {
    ...
    {
	  oldQNH  = iQNH;
      _stprintf(szTmp, TEXT("$PFLX3,%i,,,,,,,,,,,,"), (int)Units::ToUser(unFeet, QFEAltitudeOffset-(double)LX16xxAlt) );
      LX16xxNMEAddCheckSumStrg(szTmp);
...
bool DevLX16xx::LXWP0(DeviceDescriptor_t* d, const char* sentence, NMEA_INFO* info)
{
  ...
  {
    info->ExternalWindSpeed = Units::ToSys(unKiloMeterPerHour, info->ExternalWindSpeed);  /* convert to m/s */
