...
double PirkerAnalysis(NMEA_INFO* Basic, DERIVED_INFO* Calculated,
  ...
  while (pirker_mc < max_mc) {
    h = GlidePolar::MacCreadyAltitude(pirker_mc,
                                      1.0,  // unit distance
                                      this_bearing, Calculated->WindSpeed,
                                      Calculated->WindBearing, 0, 0, true, 0);
    ...
    // Paraglider's polar is problematic, since everything happens at very close
    // speeds. So we iterate more times. Because ArrivalAltitude at MC 5 can be
    // the same at MC 5.5, at MC6.0.. but different at MC 7.0.
