...
COLORREF InfoBox::amberColor = RGB(0xff,0xbe,0x00);
...
InfoBox::InfoBox(HWND Parent, int X, int Y, int Width, int Height){
  ...
  if (Count == 0){
	...
	if (Appearance.InverseInfoBox) 
		...
		hBrushDefaultBackGround = LKBrush_White;

    hBrushDefaultBackGroundSel = LKBrush_DarkGreen;
...
// VENTA3 added colors
  ...
  if ((mBorderKind & BORDERTAB) && (halftextwidth>NIBLSCALE(3))) {

    int ytop = recTitle.top + (mpFontHeightTitle->CapitalHeight)/2; 
    int ytopedge = ytop+NIBLSCALE(2);
    int ybottom = recTitle.top + NIBLSCALE(6) + mpFontHeightTitle->CapitalHeight;

    HPEN oldPen = (HPEN)SelectObject(mHdcBuf, mhPenBorder);

    POINT tab[8];
    tab[0].x = tab[1].x = recTitle.left+NIBLSCALE(1);
    tab[0].y = tab[7].y = ybottom;
    tab[2].x = recTitle.left+NIBLSCALE(3);
    tab[2].y = tab[5].y = tab[3].y = tab[4].y = ytop;
    tab[1].y = tab[6].y = ytopedge;
    tab[5].x = recTitle.right-NIBLSCALE(4);
    tab[6].x = tab[7].x = recTitle.right-NIBLSCALE(2);
    tab[3].x = recTitle.left+halftextwidth-NIBLSCALE(1);
    tab[4].x = recTitle.right-halftextwidth+NIBLSCALE(1);
...
void InfoBox::PaintValue(void){
  ...
  switch (color) {
  case -1:
    if (Appearance.InverseInfoBox) // 091228
    	SetTextColor(mHdcBuf, amberColor);
    else
    	SetTextColor(mHdcBuf, bdColor); 
    break;
  case 0:
