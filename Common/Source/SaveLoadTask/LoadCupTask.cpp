    ...
    if(  szTaskStrings[ iNO_Tasks] != NULL)
    {
      if(bClosedTask)
      {
          if (gTaskType == TSK_AAT)
              lk::snprintf(szTaskStrings[iNO_Tasks], READLINE_LENGTH, _T("[AAT %.1f%s] %s"), Units::ToUserDistance(lengthtotal), Units::GetDistanceName(), szString); // _@M699_ "Task"
          else if (CALCULATED_INFO.TaskFAI)
              lk::snprintf(szTaskStrings[iNO_Tasks], READLINE_LENGTH, _T("[FAI %s %.1f%s] %s"), MsgToken<2432>(), Units::ToUserDistance(lengthtotal), Units::GetDistanceName(), szString); // _@M2432_ "Triangle"
          else
              lk::snprintf(szTaskStrings[iNO_Tasks], READLINE_LENGTH, _T("[%s %.1f%s] %s"), TaskTypeLabel(NoPts), Units::ToUserDistance(lengthtotal), Units::GetDistanceName(), szString);
      }
      else
      {
        lk::snprintf(szTaskStrings[ iNO_Tasks] ,READLINE_LENGTH, _T("[%s %.1f%s] %s"), MsgToken<2429>() ,Units::ToUserDistance(lengthtotal), Units::GetDistanceName(), szString);
      }
    }
    UnlockTaskData();
    iNO_Tasks++;
