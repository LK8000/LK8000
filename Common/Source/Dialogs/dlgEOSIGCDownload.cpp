...
void AddEOSElement(TCHAR Line1[], TCHAR Line2[]) {
...
static uint8_t RecBinBlock(DeviceDescriptor_t *d, FILE *pf_IGCFile, uint16_t Sequence) {
...
uint16_t error= REC_NO_ERROR;
  ...
  switch (EOS_ThreadState)
  {
    ...
    case READRECORD_STATE_RX: 
      if(EOSBlockReceived())
      {
        error = RecBinBlock(d, pf_IGCFile, BlockNo);
