...
void NextPage(WndForm* pForm, int Step) {
  if (!pForm) {
    return;
  }
  ...
  auto wDetailsEntry = pForm->FindByName<WndOwnerDrawFrame>(_T("frmDetailsEntry"));
  if (!wDetailsEntry) {
    return;
  }
  ...
  auto wDetails = pForm->FindByName<WndListFrame>(_T("frmDetails"));
  if (wDetails) {
    wDetails->ResetList();
    wDetails->Redraw();
  }
...
void OnPaintDetailsListItem(WndOwnerDrawFrame* Sender, LKSurface& Surface) {
  (void)Sender;
...
void OnDetailsListInfo(WndListFrame* Sender, WndListFrame::ListInfo_t* ListInfo) {
...
bool FormKeyDown(WndForm* pWnd, unsigned KeyCode) {
  WindowControl* pBtn = nullptr;

  switch (KeyCode & 0xffff) {
    case KEY_LEFT:
    case '6':
      pBtn = pWnd->FindByName(_T("cmdPrev"));
      NextPage(pWnd, -1);
      break;
    case KEY_RIGHT:
    case '7':
      pBtn = pWnd->FindByName(_T("cmdNext"));
      NextPage(pWnd, +1);
      break;
  }
  if (pBtn) {
...
void addChecklist(const TCHAR* name, const TCHAR* details) {
  checklist_data.push_back({name, details});
}

//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
/// Add a line pointed to by TempString into checklist.
void AddChecklistLine(const TCHAR* TempString, TCHAR* Details, TCHAR* Name, bool& inDetails) {
  ...
  if ((_tcslen(Details) + len) > (MAXNOTEDETAILS - MAXNOTELIMITER - 1)) {
    ...
    _tcscat(Details, _T(NOTECONTINUED));
...
bool LoadChecklist(short checklistmode) {
  ...
  switch (checklistmode) {
    ...
    case 0:
      ...
      LocalPath(filename, _T(LKD_CONF), _T(LKF_CHECKLISTDEMO));
...
void dlgChecklistShowModal(short checklistmode) {
  ...
  auto wndClose = wf->FindByName<WndButton>(_T("cmdClose"));
  if (!wndClose) {
  ...
  auto wDetailsEntry = wf->FindByName<WndOwnerDrawFrame>(_T("frmDetailsEntry"));
  if (!wDetailsEntry) {
