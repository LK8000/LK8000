...
inline bool is_valid_jpeg(const uint8_t* d, size_t n) {
  ...
  while (i < n) {
    ...
    // SOS (Start of Scan) - marks start of entropy-coded image data
    // Valid header up to SOS is sufficient for JPEG detection;
    // stop parsing to avoid unbounded scan through entropy data
    if (marker == 0xDA) {
      return true;
    }
