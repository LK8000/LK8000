...
TEST_CASE("testing the StrToDouble function") {
	...
	SUBCASE("Valid literal string imput") {
		CHECK(StrToDouble(_T("0"), nullptr) == 0);
		CHECK(StrToDouble(_T("1"), nullptr) == 1);
		CHECK(StrToDouble(_T("2.2"), nullptr) == 2.2);
		CHECK(StrToDouble(_T("1234567890.0123456789"), nullptr) == 1234567890.0123456789);
		CHECK(StrToDouble(_T("-1.1"), nullptr) == -1.1);
		CHECK(StrToDouble(_T("  -1.1"), nullptr) == -1.1);
		CHECK(StrToDouble(_T("  1.1"), nullptr) == 1.1);
		CHECK(StrToDouble(_T("0.1"), nullptr) == 0.1);
		CHECK(StrToDouble(_T(".1"), nullptr) == 0.1);
		CHECK(StrToDouble(_T("6.6657667387"), nullptr) == 6.6657667387);

		const TCHAR* sz = nullptr;
		CHECK(StrToDouble(_T(".1"), &sz) == 0.1);
		CHECK((sz && sz[0] == '\0') == true);
	}

	SUBCASE("partial Valid literal string imput") {
		CHECK(StrToDouble(_T("0a"), nullptr) == 0);
		CHECK(StrToDouble(_T("1a"), nullptr) == 1);
		CHECK(StrToDouble(_T("2.2a"), nullptr) == 2.2);
		CHECK(StrToDouble(_T("1234567890.0123456789a"), nullptr) == 1234567890.0123456789);
		CHECK(StrToDouble(_T("-1.1a"), nullptr) == -1.1);

    const TCHAR *sz = nullptr;
    CHECK(StrToDouble(_T("0a"), &sz) == 0);
    CHECK((sz && sz[0] == 'a') == true);

    CHECK(StrToDouble(_T("1b"), &sz) == 1);
    CHECK((sz && sz[0] == 'b') == true);

    CHECK(StrToDouble(_T("2.2c"), &sz) == 2.2);
    CHECK((sz && sz[0] == 'c') == true);

    CHECK(StrToDouble(_T("1234567890.0123456789d"), &sz) == 1234567890.0123456789);
    CHECK((sz && sz[0] == 'd') == true);

    CHECK(StrToDouble(_T("-1.1e"), &sz) == -1.1);
    CHECK((sz && sz[0] == 'e') == true);
  }

	SUBCASE("invalid literal string imput") {
		const TCHAR* sz = nullptr;
		CHECK(StrToDouble(_T("a"), &sz) == 0);
		CHECK((sz && sz[0] == 'a') == true);

		CHECK(StrToDouble(_T(" ba"), &sz) == 0);
		CHECK((sz && sz[0] == 'b') == true);

		CHECK(StrToDouble(_T(",2"), &sz) == 0);
		CHECK((sz && sz[0] == ',') == true);
	}
