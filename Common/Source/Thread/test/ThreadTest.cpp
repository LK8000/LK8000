...
TEST_SUITE("Thread") {
  ...
  TEST_CASE("ThreadHandle") {
    auto mainthread = ThreadHandle::GetCurrent();
    CHECK(mainthread.IsInside());

    InvokeThread Thread("test", [&mainthread]() {
      CHECK_FALSE(mainthread.IsInside());
      auto current = ThreadHandle::GetCurrent();
      CHECK(current.IsInside());
    });
    CHECK(Thread.Start());
    Thread.Join();
  }
