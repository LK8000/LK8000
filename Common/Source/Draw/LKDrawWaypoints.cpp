		...
		if( intask || irange || dowrite) {  // irange always set when MapScale <=10
			...
			if (draw_alt) {
				switch ( ArrivalValue ) {
					...
					case avAltitude :
						_stprintf(Buffer, TEXT("%s:%d%s"), Buffer2, (int)Units::ToUserAltitude(tp.AltArivalAGL), sAltUnit);
					break;

					case avGR_Altitude :
						_stprintf(Buffer, TEXT("%s:%d/%d%s"), Buffer2, (int)tpc.GR, (int)Units::ToUserAltitude(tp.AltArivalAGL), sAltUnit);
					break;
				}
		...
		if (dowrite) {
			...
			if (PtInRect(&ClipRect, LabelPos)) {
				...
				MapWaypointLabelAdd(
						...
						LabelPos,
						&TextDisplayMode,
						(int) Units::ToUserAltitude(tp.AltArivalAGL),
