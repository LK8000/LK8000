    ...
    for (const auto& pAsp : airspaces_to_draw) {
      if (pAsp->WarningLevel() > awNone) {
        ...
        if (distances_ready && (hlabeldrawstyle > awsHidden) && PointVisible(lon, lat)) {
          ...
          else if (TextDisplayMode.Color == RGB_BLACK) {
        ...
        if (distances_ready && vlabeldrawstyle > awsHidden) {
          ...
          vlabel_draws =
              TextInBox(Surface, &rc, hbuf, Orig_Aircraft.x, Orig_Aircraft.y + NIBLSCALE(15), &TextDisplayMode, true);
        }
        if (!label_sequencing_divider)
          CAirspaceManager::Instance().AirspaceWarningLabelPrinted(*pAsp, hlabel_draws || vlabel_draws);
