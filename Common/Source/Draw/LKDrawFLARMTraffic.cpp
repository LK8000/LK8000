  ...
  for (i=0,painted=0; i<FLARM_MAX_TRAFFIC; i++) {
	...
	if ( (DrawInfo.FLARM_Traffic[i].RadioId != 0) && (DrawInfo.FLARM_Traffic[i].Status != LKT_ZOMBIE) && (DrawInfo.FLARM_Traffic[i].Speed > 0)){
		...
		if (DrawInfo.FLARM_Traffic[i].Average30s>=0.1) {
                  ...
                  if (len > 0)
                    lk::snprintf(lbuffer + len, std::size(lbuffer) - len, _T(":%.1f"),Units::ToVerticalSpeed(DrawInfo.FLARM_Traffic[i].Average30s));
                  else
                    lk::snprintf(lbuffer,_T("%.1f"),Units::ToVerticalSpeed(DrawInfo.FLARM_Traffic[i].Average30s));
		}
