...
DiagrammStruct sDia;
  ...
  DrawXGrid(Surface, rc34, Units::ToSysDistance(xtick), xtick, 0,TEXT_ABOVE_LEFT, rgbGridColor,  &sDia, text);
...
double scl = xtick;
	...
	{
    iCircleRadius =(int) (fRing* fCScale / Units::ToUserDistance(1000.0f));
...
if(SPLITSCREEN_FACTOR >0)
{
	...
	#endif
	  {
	    ...
	    switch (LKTraffic[i].Status) { // 100321
		  ...
		  default:
		     ...
		     * draw label
		     *************************************************************************/
		   _stprintf(lbuffer,_T("%3.1f"),Units::ToUserVerticalSpeed(LKTraffic[i].Average30s));
 ...
 // sDia.rc = rc34;
  if(bSideview)
  {
    ...
    DrawYGrid(Surface, rc, Units::ToSysAltitude(ytick), ytick, 0,TEXT_UNDER_RIGHT , rgbGridColor, &sDia, text);
  }
