...
Kobo-update-otg.zip: $(BIN)/otg-update/.kobo/KoboRoot.tgz
	@$(NQ)echo "  Zip     $@"
	$(Q)cd $(BIN)/otg-update/ && zip -q $@ .kobo/KoboRoot.tgz
	$(Q)cp $(BIN)/otg-update/$@ $@


Kobo-update.zip: $(BIN)/std-update/.kobo/KoboRoot.tgz
	@$(NQ)echo "  Zip     $@"
	$(Q)cd $(BIN)/std-update/ && zip -q $@ .kobo/KoboRoot.tgz
	$(Q)cp $(BIN)/std-update/$@ $@
...
$(BIN)/std/.kobo/KoboRoot.tgz: $(OUTPUTS) $(KOBO_MENU_BIN) $(KOBO_POWER_OFF_BIN) \
	...
	$(Q)install -m 0644 $(WAYPOINT_FILES) $(BIN)/std/KoboRoot/mnt/onboard/LK8000/_Waypoints
	$(Q)install -m 0644 $(CONFIG_FILES) $(BIN)/std/KoboRoot/mnt/onboard/LK8000/_Configuration
...
$(BIN)/otg/.kobo/KoboRoot.tgz: $(OUTPUTS) $(KOBO_MENU_BIN) $(KOBO_POWER_OFF_BIN) \
	...
	$(Q)install -m 0644 $(WAYPOINT_FILES) $(BIN)/otg/KoboRoot/mnt/onboard/LK8000/_Waypoints
	$(Q)install -m 0644 $(CONFIG_FILES) $(BIN)/otg/KoboRoot/mnt/onboard/LK8000/_Configuration
...
$(BIN)/std-update/.kobo/KoboRoot.tgz: $(OUTPUTS) $(KOBO_MENU_BIN) $(KOBO_POWER_OFF_BIN) \
	$(SYSTEM_FILES) $(BITMAP_FILES) \
	$(FONTS_FILES) $(POLAR_FILES) $(LANGUAGE_FILES) \
	$(CONFIG_FILES)  kobo/inittab kobo/rcS
	@$(NQ)echo "  TAR     $@"
	
	$(call build-kobofs,std-update)
	
	$(Q)install -m 0755 -d  $(BIN)/std-update/.kobo
	$(Q)fakeroot tar czfC $@ $(BIN)/std-update/KoboRoot .

$(BIN)/otg-update/.kobo/KoboRoot.tgz: $(OUTPUTS) $(KOBO_MENU_BIN) $(KOBO_POWER_OFF_BIN) \
	$(SYSTEM_FILES) $(BITMAP_FILES) \
	$(FONTS_FILES) $(POLAR_FILES) $(LANGUAGE_FILES) \
	$(CONFIG_FILES)  kobo/inittab kobo/rcS kobo/60-persistent-serial.rules $(KOBO_KERNEL_SOURCES)
	@$(NQ)echo "  TAR     $@"
	
	$(call build-kobofs,otg-update)

	$(Q)install -m 0755 -d  $(BIN)/otg-update/KoboRoot/lib/udev/rules.d
	$(Q)install -m 0644 kobo/60-persistent-serial.rules $(BIN)/otg-update/KoboRoot/lib/udev/rules.d
	$(Q)install -m 0755 -d  $(BIN)/otg-update/KoboRoot/mnt/onboard/LK8000/kobo
	$(Q)install -m 0644 $(KOBO_KERNEL_SOURCES) $(BIN)/otg-update/KoboRoot/mnt/onboard/LK8000/kobo

	$(Q)install -m 0755 -d  $(BIN)/otg-update/.kobo
	$(Q)fakeroot tar czfC $@ $(BIN)/otg-update/KoboRoot .
